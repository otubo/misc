...
"LEFT_MASTER")
			...
			#make sure that there are no maximized windows, otherwise tiling would fail.
...
"TOP_MASTER")
			...
			wmctrl	-r :ACTIVE: -e "0,$X,$Y,$W,$H"
			 ...
			 #Distribute the  width among the remaining windows
...
"ACTIVE_LEFT")
				...
				Y=$TOP_MARGIN
				H=$((HEIGHT-TOP_MARGIN-BOTTOM_MARGIN))
				W=$(((WIDTH-LEFT_MARGIN-RIGHT_MARGIN)/2))
				#set the active window to the "master "area
				wmctrl -r :ACTIVE: -b remove,maximized_vert,maximized_horz
				wmctrl	-r :ACTIVE: -e "0,$X,$Y,$W,$H"
				 ;;					 
