...
while test ! ${LAST_TS_FILE}; do
	...
	timeout -k 1m ${EPISODE_SIZE} ${FFMPEG} -i ${URL} -c copy -strict 2 \
		${BASE}${TIMESTAMP}.ts >/dev/null 2>&1
...
${FFMPEG} -i "${LAST_TS_FILE}" -acodec mp3 -write_xing 0 "${NEW_MP3_FILE}" >/dev/null 2>&1

# set id3 tags
${ID3} -t "$(date)" "${NEW_MP3_FILE}" >/dev/null 2>&1
${ID3} -a ${DATE} "${NEW_MP3_FILE}" >/dev/null 2>&1
rm "${LAST_TS_FILE}" >/dev/null 2>&1

# upload to my spreaker account
curl -X POST -H "Authorization: Bearer ${OAUTH_TOKEN}" -F media_file=@${NEW_MP3_FILE} -F \
	"title=${DATE}" https://api.spreaker.com/v2/shows/${SHOW_ID}/episodes >/dev/null 2>&1
...
if [ ${N_EPISODES_ONLINE} -lt 2 ]; then
	...
	curl -X DELETE -H "Authorization: Bearer ${OAUTH_TOKEN}" \
		https://api.spreaker.com/v2/episodes/${OLDEST} >/dev/null 2>&1;
fi
